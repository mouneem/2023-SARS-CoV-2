...
write.csv(dfg, '~/Projects/SARS-CoV-2 2023/dfg.csv')


colors <- read.csv('~/Projects/SARS-CoV-2 2023/color_s.csv')
rownames(colors) <- colors$node
g <- igraph::graph.data.frame(dfg, directed = FALSE )
V(g)$color <- colors[V(g)$name,'color']

plot(g, vertex.color = V(g)$color, layout = layout.kamada.kawai(g) )

plot(x = g,
    layout = igraph::layout.fruchterman.reingold(g),
    edge.weight = V(g)$weight, 
    vertex.size=5,
    scale=c(2,2),
    #mar=c(0,0,0,0)
...
plot(x = g,
    layout = igraph::layout.kamada.kawai(g),
    edge.weight = V(g)$weight, 
    vertex.size=5,
    scale=c(2,2),
    #mar=c(0,0,0,0)
